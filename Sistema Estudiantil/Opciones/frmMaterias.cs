...
namespace Sistema_Estudiantil.Opciones
{
    public partial class frmMaterias : Form
    {
        ...
        void CargarDatos()
        {
            ...
            {
                var lista = context.materias.ToList();
                dgvElementos.DataSource = context.materias.Local.ToBindingList();
        ...
        void Guardar()
        {
            using (var context = new programacion3Entities())
            {
                var std = new materias()
                {
                    materia = txtAgregar.Text
                };
                context.materias.Add(std);
                context.SaveChanges();
            }
            CargarDatos();
        }
        ...
        void Filtrar()
        {
            using (var context = new programacion3Entities())
            {
                var lista = context.materias.Where(s => s.materia.Contains(txtBuscar.Text)).ToList();
                dgvElementos.DataSource = context.materias.Local.ToBindingList();
            }
        }
